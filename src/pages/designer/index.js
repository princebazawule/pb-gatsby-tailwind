...
export default function Designer({ data }) {
  ...
  const defaultImageSrc = selectDefaultImage[0].node.childImageSharp.fluid.src
    ...
    return (
        <>
            <Layout>
              <Seo title="Designer" keywords={[`ui design`, `digital art`, `graphic design`, `design`, `art`, `artist`]} description={`Prince Bazawule - Award-winning multi-disciplinary designer`} image={`${siteUrl}${defaultImageSrc}`} url={`${siteUrl}/designer/`} />
