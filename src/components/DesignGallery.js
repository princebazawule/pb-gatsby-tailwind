...
export default function DesignGallery( { data }) {
    ...
    return (
        <section
            ref={galleryRef}
            className='gallery relative rounded-lg px-2 lg:px-2 3xl:px-10 mt-24 sm:mt-20 md:mt-24 lg:mt-32 xl:mt-0 xl:right-10 2xl:right-12 3xl:right-56 4xl:right-72 xl:absolute xl:top-1/2 xl:transform xl:-translate-y-1/2 xl:w-1/4 2xl:w-1/4 xl:h-3/5 xl:transform-gpu xl:-rotate-3'
        ...
        >
            <div className='gallery-scroller flex flex-row xl:flex-col justify-start xl:content-center flex-nowrap w-full overflow-x-scroll xl:overflow-y-scroll h-full'>
                {designs.map(design => {
                    return (
                        <Link
                            to={`/designs/${design.frontmatter.slug}`}
                            key={design.id}
                        ...
                        >
                            <div className="group project-item transition duration-300 scale-100 hover:scale-105 4xl:scale-110 4xl:hover:scale-y-125 flex flex-col justify-center items-center relative">
                                ...
                                <h4 className='absolute z-30 text-2xl 2xl:text-xl text-gray-900 dark:text-white font-bold transition duration-300'>{design.frontmatter.title}</h4>
